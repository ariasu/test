...
path = './code_file/csv_data/'
kakutyo = '.csv'

df_jp_ind = pd.read_csv(path + '雇用_医療福祉_一人当たり賃金_全国_全産業' + kakutyo, encoding='shift_jis')
df_jp_cate = pd.read_csv(path + '雇用_医療福祉_一人当たり賃金_全国_大分類' + kakutyo, encoding='shift_jis')
df_pref_ind = pd.read_csv(path + '雇用_医療福祉_一人当たり賃金_都道府県_全産業' + kakutyo, encoding='shift_jis')

st.header('ヒートマップ')

jp_lat_lon = pd.read_csv('pref_lat_lon' + kakutyo)
...
df_pref_map['一人当た当たりの賃金（相対値）'] = (df_pref_map['一人当たり賃金（万円）'] - df_pref_map['一人当たり賃金（万円）'].min()) / (df_pref_map['一人当たり賃金（万円）'].max() - df_pref_map['一人当たり賃金（万円）'].min())
...
max_x = df_mean_cate[option_wage].max() + 50
fig = px.bar(df_mean_cate,
            x = option_wage,
            y = '産業大分類名',
            color = '産業大分類名',
            animation_frame = '年齢',
